...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const tableName = 'Photography.csv';
    
    // Get Airtable API key from environment variables
    // Prefer base-specific token if provided, then fall back to global
    const apiKey = process.env.AIRTABLE_API_KEY_PHOTOGRAPHY || process.env.AIRTABLE_API_KEY || process.env.AIRTABLE_TOKEN;
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({ 
        ...
        expectedEnvVars: ['AIRTABLE_API_KEY_PHOTOGRAPHY', 'AIRTABLE_API_KEY', 'AIRTABLE_TOKEN']
      });
    }

    const tokenType = apiKey.startsWith('pat') ? 'PAT' : (apiKey.startsWith('key') ? 'legacy' : 'unknown');
    const keySource = process.env.AIRTABLE_API_KEY_PHOTOGRAPHY ? 'AIRTABLE_API_KEY_PHOTOGRAPHY' : (process.env.AIRTABLE_API_KEY ? 'AIRTABLE_API_KEY' : 'AIRTABLE_TOKEN');

// Same-page smooth scrolling for hash links only
    ...
    // Connection hints only. Do not load partner sites in hidden iframes:
    // Kuts By Lee sets window.top.location when framed, which navigates this
    // page to kutsbylee.com. Visible previews are sandboxed in systems-live.js.
    (function warmSystemsConnections() {
      const ORIGINS = [
      ...
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', ensureHeadLinks);
      } else {
        ensureHeadLinks();
      }
    })();